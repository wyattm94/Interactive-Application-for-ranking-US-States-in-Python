...
def mkstr(*args,sep='',sep_list='',sep_tuple=' '):
    s = ''
    # print(len(args)); sys.stdout.flush();
    for i in range(len(args)):
        a = args[i]
        if isinstance(a,list):
            for e in a:
                s = s + str(str(e) + ifelse(bool((i+0)>=len(args)),'',str(sep_list)))
        elif isinstance(a,tuple):
            for e in a:
                s = s + str(str(e) + ifelse(bool((i+0)>=len(args)),'',str(sep_tuple)))
        else:
            s = s + str(str(a) + ifelse(bool((i+0)>=len(args)),'',str(sep)))
...
def p2log(*args,local=False):
    cdt = dt_tag().split('_')
    tag = mkstr('\n-----[User Log Updated: ',
                nicetime(cdt[1]),' - ',nicedate(cdt[0]),']\n')
    pconn = open('.logfile','a')
    for a in args: print(a,file=pconn)
    pconn.close()
    if local: print(*args)
...
def xb2py(p,mtmax=10):
    ...
    # Open connection to xlsx file, run the pool on it
    wb =opx.load_workbook(p)
    ws = list(wb.sheetnames)
    pool = mtpool(mtmax); out = pool.map(wb2df,[wb[s] for s in ws])
    sysout('\n[TIME] {t:5.3f} seconds'.format(t=(time.time()-t0)))
...
def showdt():
    ...
    return s.split('.')[0] + mkstr('.', s.split('.')[1][:3])
